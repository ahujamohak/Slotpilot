...
try:
    ...
    # --- TAB 5: MASTER SYSTEM DOCUMENTATION ---
    with tab5:
        st.markdown("""
# SLOTPILOT ANALYTICS & AI CO-PILOT: MASTER SYSTEM SPECIFICATION

## 1. SYSTEM ARCHITECTURE & DATA FLOW
The system operates as a real-time analytics engine and tactical co-pilot designed to evaluate slot machine payout cycles, feature density clusters, and volatility profiles without requiring spin-by-spin tracking.

* Data Source: Google Sheets ("Slot Analysis High Bet" -> "Session Log") synchronized via gspread service account authentication.
* Application Framework: Streamlit UI hosted on a single python execution script (`app.py`).
* AI Inference Engine: Groq API utilizing dynamic Llama-3/Mixtral models for real-time natural language query resolution.
* Data Persistence: Automated appending to remote Google Sheets with a local CSV backup download option.

---

## 2. DATASET SCHEMATICS & COLUMN DEFINITIONS
The system relies on 10 core fields logged per feature hit or session attempt:

1. Date: Date of record formatted as MM/DD/YYYY.
2. Day: Day of the week (Auto-generated to match system date).
3. Family: Game series classification (e.g., Dragon Link, Dragon Cash, Lightning Link).
4. Slot: Specific game title (e.g., Panda Magic, Golden Century, Happy & Prosperous).
5. Spin of feature hit: Spin count elapsed from check-in until the feature triggered.
6. Feature type: Mechanism triggered (e.g., Hold & Spin, Free Games, Major Progressive).
7. Win amount: Total monetary payout generated by the feature hit ($).
8. Win multiplier: Payout multiplier relative to base bet size (calculated as Win Amount / Base Bet).
9. Hit Number: Sequence ID of feature hit within a session (0 = Failed attempt / dead cycle, 1+ = Hit sequence).
10. Attempt Number: Check-in sequence count on that specific machine during the session (1 = First load, 2+ = Repeat attempt).

---

## 3. CORE ANALYTICAL ENGINES & MATHEMATICAL RULES

### A. Feature-Yield Rolling Volatility Index (Feature RVI)
To avoid manual tracking of non-winning spins, RVI measures feature yield variance directly from logged feature multipliers.

* Formula: Sample Standard Deviation of logged win multipliers for a machine or family series.
* RVI Classifications:
  * High RVI (StdDev > 45.0): Wild payout swings (e.g., 5x, 180x, 10x, 220x). Machine displays high breakout potential; triggers aggressive or scaling bet structures.
  * Moderate RVI (StdDev 20.0 to 45.0): Balanced payout distribution. Standard flat mid-tier betting applied.
  * Low RVI (StdDev < 20.0): Flat or compressed feature wins (e.g., 15x, 18x, 22x). Indicates flat bleed risk with low breakout probability.

### B. Softened Cycle Spin Target (Target Runway)
Calculates the target spin window required to hit a feature based on historical density rather than simple averages.

1. Modal Midpoint: Calculates the 10-spin bucket containing the highest frequency of feature hits (e.g., 10-20 spin bucket -> 15).
2. Raw Target: Weighted calculation combining Modal Midpoint (60%) and Overall Median Hit Spin (40%).
3. Softened Buffer: Raw Target multiplied by 1.12 (adding a 12% tolerance buffer to account for variance).
4. Minimum Threshold: Target cycle floor is hard-coded to a minimum of 15 spins.

### C. Dynamic Stop-Loss & Capital Risk Matrix
Protects bankroll by setting automated exit triggers before starting a session:

1. Initial Check-In Allocation: Calculated as Target Cycle Spins x Base Bet x 1.8, rounded up to the nearest $50 increment (Minimum check-in: $300).
2. Base Stop Floor (Hard Floor): Set at 70% of Initial Check-In (Maximum drawdown per check-in is 30%).
3. Dead-Spin Cutoff: Maximum consecutive non-winning or cold spins allowed before aborting early. Calculated as 50% of the Softened Target Cycle (Minimum floor: 12 spins).
4. Trailing Profit Lock Floor: Set at 140% of Initial Check-In. Once reached, bankroll floor moves up to lock in profit.

### D. AI Bet Strategy Selection Logic
Dynamically maps game statistics to bet sizing and denomination selection:

* Flat High-Bet ($10.00): Triggered if Median Hit Spin <= 30 AND (RVI >= 25 OR sample size <= 2) AND Average Multiplier >= 40.
* Varying / Scaling ($2.50 to $7.50): Triggered if RVI > 40.0 AND Max Historical Multiplier >= 80x.
* Flat Low-Bet ($2.50): Triggered if Median Hit Spin > 45 AND Average Multiplier < 35x.
...
### Tab 1: Live Feature Logger
* Input interface optimized for rapid logging on mobile or desktop.
* Automatically converts selected calendar dates to strict day strings and formatted date strings.
* Features custom input fields for new game titles or machine families.
* Includes a global session log CSV export backup button.
...
* Yield Heatmap: Matrix charting Day of the Week vs Family payout performance.
* Cumulative Trigger Probability (CDF): Cumulative distribution histogram of feature spin intervals.
* Multiplier Box Plot: Distribution spread showing median, quartile, and outlier payouts per family.

### Tab 4: Today's Priority Board
* Filters session data dynamically by current Day of the Week.
* Ranks all historical family and slot combinations using a Composite Priority Score:
  Composite Score = (Avg Multiplier x 0.35) + (RVI Score x 0.25) + (Total Hits x 2.0) + (Base Bet x 2.0) - (Target Cycle x 0.15)
* Outputs top 15 priority machines complete with bet configurations, RVI metrics, stop-loss floors, and dead-spin limits.
* Features a single-click push mechanism to pass top choices straight into the Live Co-Pilot.

---

## 5. SYSTEM CONSTRAINTS & GUARANTEES
1. Non-Per-Spin Tracking Guarantee: The user is never required to log individual non-winning spins. All metrics derive strictly from feature hit logs and check-in amounts.
2. Direct System Prompting: AI outputs are constrained to strictly remove internal reasoning tags (<think> blocks) and conversational filler.
3. Fallback Resiliency: If day-specific historical records fall below 5 hits, the system automatically expands its data sample to the full cross-day dataset to maintain statistical validity.
        """)
